...
# Вспомогательная функция для получения путей в PyInstaller (ИСПРАВЛЕНА)
def resource_path(relative_path):
    """ Получает абсолютный путь к ресурсу, работает для разработки и для PyInstaller """
    try:
        base_path = sys._MEIPASS
    except Exception:
        # Теперь Python всегда будет искать файлы строго рядом с самим скриптом
...
def run_full_caisr_pipeline(
    ...
    # =========================================================================
    # ЗАПУСК DOCKER PIPELINE (Stage Only)
    # =========================================================================
    print(f"\n[CAISR DEBUG] LAUNCHING DOCKER PIPELINE (Stage Only)...")
    try:
        ...
        print("[CAISR DEBUG] Docker pipeline completed successfully.")
    except Exception as e:
        print(f"[CAISR ERROR] Docker script failed with error: {e}")
        import traceback
        traceback.print_exc()
