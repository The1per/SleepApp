...
class StdoutRedirector:
    ...
    def write(self, message):
        # Пишем в невидимую консоль для стабильности
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.docx_report_type = tk.StringVar(value="detailed")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(meta, text="EEG electrode set:").grid(row=0, column=0, sticky="w")
        montage_box = ttk.Combobox(meta, textvariable=self.montage_display, values=["Adults E256", "Kids E128"], width=16, state="readonly")
        montage_box.grid(row=0, column=1, sticky="w", padx=(8, 25))
        montage_box.bind("<<ComboboxSelected>>", lambda e: self._on_montage_changed())

        ttk.Label(meta, text="Patient Age:").grid(row=0, column=2, sticky="w")
        age_spin = ttk.Spinbox(meta, textvariable=self.subject_age, from_=1, to=120, increment=1, width=7, font=(FONT_FAMILY_UI, FONT_SIZE_BODY))
        age_spin.grid(row=0, column=3, sticky="w", padx=(8, 25))

        ttk.Label(meta, text="Patient Sex:").grid(row=0, column=4, sticky="w")
        sex_box = ttk.Combobox(meta, textvariable=self.sex, values=["F", "M"], width=8, state="readonly")
        sex_box.grid(row=0, column=5, sticky="w", padx=(8, 25))

        ttk.Label(meta, text="Report type:").grid(row=0, column=6, sticky="w")
        docx_type_box = ttk.Combobox(meta, textvariable=self.docx_report_type, values=["simple", "detailed"], width=10, state="readonly")
        docx_type_box.grid(row=0, column=7, sticky="w", padx=(8, 25))

        ttk.Label(meta, text="Primary Scoring System:").grid(row=0, column=8, sticky="w")
        scoring_box = ttk.Combobox(meta, textvariable=self.primary_scoring, values=["YASA", "CAISR"], width=8, state="readonly")
        scoring_box.grid(row=0, column=9, sticky="w", padx=(8, 0))
        ...
        self.progress = ttk.Progressbar(bar_row, mode="indeterminate", length=180)
    ...
    def validate_scoring_channel(self, mff_path: str, montage_type: str, side: str, selected_site: str) -> tuple[bool, str]:
        ...
        try:
            ...
            return True, f"Validation skipped (Error: {e})"
    ...
    def _run_pipeline_compat(self, *, mff_paths: list, montage_type: str, side: str, site: str,
                             ...
                             do_docx: bool, age_val: int | None, sex: str | None, docx_type: str, append_pdf: str | None, primary_scoring: str):
        ...
        if do_docx:
            if "docx_report_type" in params: kwargs["docx_report_type"] = docx_type
            elif "docxreporttype" in params: kwargs["docxreporttype"] = docx_type
            elif "report_type" in params: kwargs["report_type"] = docx_type
    ...
    def run_pipeline(self) -> None:
        ...
        docx_type = self.docx_report_type.get().strip().lower() or "simple"
        montage_type = self.get_montage_type()
        ...
        def worker() -> None:
            ...
            try:
                ...
                res = self._run_pipeline_compat(
                    ...
                    docx_type=docx_type, append_pdf=append_pdf, primary_scoring=primary_scoring
                )
                ...
                def report_success() -> None:
                    ...
                    messagebox.showinfo("Done", f"Analysis completed successfully!\n\nResults generated in:\n{outdir}")
