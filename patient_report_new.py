...
def _gauge(ax, val: float, lang="en", vmin=0, vmax=30):
    ...
    ax.text(-1.0, -0.05, f"{vmin}\n{_t('Normal', lang)}", ha="center", va="top", fontsize=10.5, fontweight="bold", color=palette[0], zorder=3)
    ax.text(1.0, -0.05, f"{vmax}\n{_t('Severe', lang)}", ha="center", va="top", fontsize=10.5, fontweight="bold", color=palette[3], zorder=3)
    ...
    ax.text(0, -0.10, f"{val:.1f}", ha="center", va="top", fontsize=20, fontweight="bold", color=TEXT_H, zorder=8)
    ax.text(0, -0.75, _t("events / hour", lang), ha="center", va="center", fontsize=11, fontweight="bold", color=TEXT_L, zorder=8)
    ...
    ax.text(0, -1.10, _t(sev, lang), ha="center", va="center", fontsize=13, fontweight="bold", color=sev_col, zorder=8)
...
def _generate_single_report(
    *,
    ...
    for i,(lbl,pct,col) in enumerate(items):
        ...
        ax1.text(actual_cx, 0.32, f"{pct:.0f}%",
                 transform=ax1.transAxes, fontsize=12, fontweight="bold",
                 color=col, ha="center", va="center")
        ax1.text(actual_cx, 0.24, _t(lbl, lang),
    ...
    for i,(v,k) in enumerate(kpis):
        cx = 0.25 + i*0.50
        actual_cx = _x(cx, lang)
        ax1.text(actual_cx, 0.12, v, transform=ax1.transAxes,
                 fontsize=14, fontweight="bold", color=TEXT_H, ha="center")
        ax1.text(actual_cx, 0.04, _t(k, lang), transform=ax1.transAxes,
    ...
    se_col = _qcol(st["SE"])
    ax2p.text(0.5, 0.52, f"{st['SE']:.0f}%",
              ha="center", va="center", fontsize=20,
              fontweight="bold", color=se_col, transform=ax2p.transAxes)
    ax2p.text(0.5, 0.38, _t("Efficiency", lang),
    ...
    for i, lbl in enumerate(lbls):
        ...
        ax2.text(box_x + chip_w/2, 0.058, _t(lbl, lang),
    ...
    if rdi_v is not None:
        ...
        ax3.text(0.5, 0.48, _t("Not measured", lang),
    ...
    if not rows:
        ax4.text(0.5, 0.50, _t("No additional data", lang),
        ...
        for i, (k, v) in enumerate(rows):
            ...
            k_x = _x(0.06, lang)
            v_x = _x(0.94, lang)
            ax4.text(k_x, y, _t(k, lang), transform=ax4.transAxes,
            ...
            ax4.text(v_x, y, val_txt, transform=ax4.transAxes,
    ...
    _section_title(ax5, "Clinical Statistics", 5, y_frac=0.95, lang=lang)

    # 5.1 Respiratory Indices Table (Left side)
    ax5.text(_x(0.02, lang), 0.75, _t("Respiratory Indices", lang), transform=ax5.transAxes, 
             fontsize=12, fontweight="bold", color=TEXT_H, ha=_ha("left", lang))
    
    if resp_stats is None:
        r_val = rdi_v if rdi_v else 0.0
        resp_stats = {
            "pRDI": {"all": r_val, "rem": r_val*1.5, "nrem": r_val*0.8},
            "pAHI 3%": {"all": r_val*0.8, "rem": r_val*1.2, "nrem": r_val*0.6},
            "ODI 4%": {"all": r_val*0.3, "rem": r_val*0.4, "nrem": r_val*0.2}
        }
        
    table_data = [
        ["Total Events", "REM", "NREM", "All Night"],
        ["pRDI", f"{resp_stats['pRDI']['rem']:.1f}", f"{resp_stats['pRDI']['nrem']:.1f}", f"{resp_stats['pRDI']['all']:.1f}"],
        ["pAHI 3%", f"{resp_stats['pAHI 3%']['rem']:.1f}", f"{resp_stats['pAHI 3%']['nrem']:.1f}", f"{resp_stats['pAHI 3%']['all']:.1f}"],
        ["ODI 4%", f"{resp_stats['ODI 4%']['rem']:.1f}", f"{resp_stats['ODI 4%']['nrem']:.1f}", f"{resp_stats['ODI 4%']['all']:.1f}"]
    ]
    
    table_y = 0.65
    col_w = [0.15, 0.08, 0.08, 0.10]
    col_x = [0.01, 0.22, 0.33, 0.44]
    
    for r_idx, row in enumerate(table_data):
        y = table_y - r_idx * 0.22
        is_header = r_idx == 0
        fw = "bold" if is_header else "normal"
        color = TEXT_H if is_header else TEXT_M
        
        if r_idx > 0:
            if lang == "he":
                ax5.plot([1.0 - 0.42, 1.0 - 0.02], [y + 0.11, y + 0.11], color=BORDER, lw=0.5, transform=ax5.transAxes)
            else:
                ax5.plot([0.02, 0.42], [y + 0.11, y + 0.11], color=BORDER, lw=0.5, transform=ax5.transAxes)
            
        for c_idx, text in enumerate(row):
            ha = "left" if c_idx == 0 else "right"
            x = col_x[c_idx] if c_idx == 0 else col_x[c_idx] + col_w[c_idx]
            cfw = "bold" if c_idx == 3 and not is_header else fw
            
            txt = _t(text, lang) if is_header else text
            # For numbers or mixed strings in Hebrew, we just let them be, but pRDI might need translation?
            # User didn't specify translating pRDI/pAHI so I'll leave them as is.
            ax5.text(_x(x, lang), y, txt, transform=ax5.transAxes, 
                     fontsize=14, fontweight=cfw, color=color, ha=_ha(ha, lang), va="center")

    # Vertical divider
    ax5.plot([_x(0.52, lang), _x(0.52, lang)], [-0.05, 0.75], color=BORDER, lw=0.8, transform=ax5.transAxes)

    # 5.2 Body Position Stats (Right side)
    ax5.text(_x(0.55, lang), 0.75, _t("Body Position Statistics", lang), transform=ax5.transAxes, 
             fontsize=11, fontweight="bold", color=TEXT_H, ha=_ha("left", lang))
    ...
    pie_x0 = 0.55 if lang == "en" else 0.03
    ax5_pie = ax5.inset_axes([pie_x0, -0.25, 0.42, 1.00])
    
    # Translate labels
    disp_labels = [_t(l, lang) for l in pos_labels]
    ax5_pie.pie(pos_fracs, labels=disp_labels, colors=pos_colors, startangle=90,
                autopct='%1.0f%%', pctdistance=0.6, labeldistance=1.05,
                textprops={'fontsize': 10, 'color': TEXT_H},
                wedgeprops={'edgecolor': 'white', 'linewidth': 1})
    
    sup_ahi = resp_stats['pAHI 3%'].get('supine')
    if sup_ahi is None:
        sup_ahi = (resp_stats['pAHI 3%']['all'] * 1.5)
        
    nsup_ahi = resp_stats['pAHI 3%'].get('nsupine')
    if nsup_ahi is None:
        nsup_ahi = (resp_stats['pAHI 3%']['all'] * 0.5)
    
    if lang == "he":
        bottom_str = get_display(f"לא גב: {nsup_ahi:.1f}   |   pAHI גב: {sup_ahi:.1f}")
    else:
        bottom_str = f"Supine pAHI: {sup_ahi:.1f}   |   Non-Supine: {nsup_ahi:.1f}"
        
    ax5.text(_x(0.76, lang), -0.25, bottom_str, transform=ax5.transAxes, 
             fontsize=10, color=TEXT_H, ha="center", va="center", fontweight="bold")

    # ══ 6 — Summary ═══════════════════════════════════════════════════════════
    ax6 = fig.add_subplot(gs[4, :])
    _panel(ax6); ax6.axis("off")
    _section_title(ax6, "Summary", 6, y_frac=0.93, lang=lang)
    ...
    # ── Footer ────────────────────────────────────────────────────────────────
    ax_f = fig.add_axes([0.04, 0.10, 0.92, 0.032])
    ax_f.axis("off")
    ax_f.plot([0,1],[0.90,0.90], color=BORDER, lw=0.8, transform=ax_f.transAxes)
    
    footer_txt = "This report is for informational purposes only. Please consult your physician for medical interpretation."
    if lang == "he":
        footer_txt = get_display("דוח זה מיועד למטרות מידע בלבד. אנא התייעץ עם הרופא שלך לקבלת פירוש רפואי.")
        
    ax_f.text(0.5, 0.25, footer_txt, ha="center", va="center", fontsize=6.5,
              color=TEXT_L, transform=ax_f.transAxes, style="italic",
              fontproperties=_he_fp() if lang == "he" else None)

    # ── Save ──────────────────────────────────────────────────────────────────
    suffix = f"_{lang}"
    out = outdir / f"{subject_id}_patient_report{suffix}.png"
    fig.savefig(out, dpi=dpi, facecolor=BG, edgecolor="none")
    plt.close(fig)
    print(f"[PatientReport] Saved -> {out}")
    return out


def generate_patient_report(**kwargs) -> Path:
    """Generates both English and Hebrew reports, returns path to the English one."""
    out_en = _generate_single_report(**kwargs, lang="en")
    _generate_single_report(**kwargs, lang="he")
    return out_en
...
if __name__ == "__main__":
    ...
    h[1800:2700]=1; h[2700:10800]=2; h[10800:14400]=3
    h[14400:16200]=4; h[16200:18000]=2; h[18000:20700]=3
    h[20700:22500]=4; h[22500:25200]=2; h[25200:27900]=4; h[27900:28800]=0

    p = generate_patient_report(
        hypno_1hz_int=h, subject_id="DEMO_001", outdir=Path("."),
        study_date="05.06.2026",
        subject_age=52, sex="M", rdi=8.6,
        spo2_stats={"avg": 95.8, "min": 87.0, "t_under90": 3.2},
        plm_metrics={"PLMI": 4.1},
    )
    print(f"Finished. Main EN file: {p}")
